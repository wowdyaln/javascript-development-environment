...
 如果您需要代碼針對不同的用戶做出不同的響應，請將JSON從 server 注入到您的應用程序中。
...
##Production Build
---
讓我們把Webpack用於捆綁和縮小我們的應用程序代碼以進行生產。
 要開始配置我們的生產應用程序，讓我們製作一份我們的開發webpack配置文件。
 我們將其稱為webpackconfig。
刺。
JS。
 現在我們來調整一些生產設置。
...
 請記住，此設置指定應該如何生成源地圖。
 我們早先在捆綁模塊中探索了源地圖。
 讓我們將Dev工具設置更改為sourcemap，因為這是生產推薦的。
 它的構建速度稍慢，但它提供了最高質量的源代碼體驗。
 這將確保我們仍然可以在瀏覽器中看到我們的原始源代碼，即使它已被縮小，轉換和捆綁。
 這就是源地圖的美妙之處。
 我們將把我們的生產版本寫入名為dist的文件夾中，讓我們改變輸出路徑。
 在構建生產應用程序時，我們會將物理文件寫入名為dist的文件夾。
 這是一個流行的慣例，它代表分配。
 接下來，讓我們設置縮小。
 我們想要縮減我們的生產代碼，所以讓我們將第一個插件添加到插件數組中。
 我們將使用一個名為UglifyJs的插件。
 我喜歡在每個插件引用上面添加註釋。
 所以你可以看到我們打電話給webpack。
優化UglifyPlugin。
 現在我們正在調用一些特定的webpack功能，我們需要在這裡添加導入。
 在我們縮小之前，讓我們使用另一個方便的webpack插件，它可以在生成包時消除任何重複的包。
 這個插件叫做Dedupe插件。
 因此，這將查看所有我們捆綁的文件，並確保沒有重複的文件被捆綁。
 現在我們將在整個模塊中增加webpack配置並增加一些功能，但這是一個好的開始。
 現在讓我們將注意力轉移到編寫將運行我們的產品webpack config build的腳本。
 所以我們將繼續構建腳本並創建一個名為build的新文件。
JS。
 這就是運行我們的webpack構建生產所需的全部內容。
 我們正在導入我們剛剛定義的webpack，我們的生產配置，以及粉筆，以便我們可以輸出顏色。
 然後我們調用webpack並傳遞它的webpack配置。
 我們正在處理任何可能發生的錯誤，否則我們會返回零表示成功的錯誤。
 所以這很簡單，但在現實世界中，您可能希望再添加一點。
 所以讓我們稍微強化一下這個腳本。
 首先，讓我們在上面調用webpack並聲明我們正在生產模式下運行Node。
 儘管我們的設置沒有必要，但我在這裡添加了這一行，因為如果在您的應用程序中為Babble創建Dev特定配置，這非常重要。
babblerc文件。
 請參閱Babble以及您可能使用的其他庫，查找此環境變量以確定它們的構建方式。
 在我們開始運行生產版本之前，我想輸出到控制台，以便我們可以看到生產版本已經開始。
 正如你所看到的，由於我們正在進行縮小，所以生產版本需要幾秒鐘才能運行。
 所以很高興得到一些通知說它正在完成這項工作。
 我喜歡在命令行中顯示一些統計信息。
 現在，這看起來像很多代碼，並不是必需的，但是這可以確保向控制台顯示警告，錯誤和統計信息，並且在底部顯示成功消息，如果一切正常。
 所以這是為了顯示任何發生的錯誤，這是為了顯示發生的警告。
 我在這裡顯示統計信息。
 最後，我們只輸出一條消息，以便我們知道我們的生產構建已成功。
 太好了，所以這裡有相當多的代碼，但它在概念上很簡單。
 這只是運行我們的生產webpack配置。
 我在這裡添加了一些額外的好處，只是為了改善我們的經驗。
 讓我們保存我們的更改，並在下一個剪輯中讓我們通過設置自動化生產版本來嘗試此操作。
